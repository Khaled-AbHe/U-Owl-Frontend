...
export const getSingleHostTruck = async (userId: string, TruckId: string) =>
  await request(`/users/${userId}/trucks/${TruckId}`);
...
export const getSingleHostTrailer = async (userId: string, TrailerId: string) =>
  await request(`/users/${userId}/trailers/${TrailerId}`);
...
export const addVehicleToCart = async (data: { vehicleId: number; distance: number }) =>
  await request("/reservations/addVehicle", { method: "PATCH", body: JSON.stringify(data) });

export const removeOrderItem = async (orderItemId: number) =>
  await request(`/reservations/removeOrderItem/${orderItemId}`, { method: "PATCH" });

// Requêtes Location
export const createLocation = async (location: any) =>
  await request("/locations/create", {
    method: "POST",
